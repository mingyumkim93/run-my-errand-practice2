...
function ErrandDetail({ user }) {
    ...
    function fetchErrand() {
        api.errand.fetchAErrand(window.location.pathname).then(res => {
            if (res.data.length === 0) { alert("Not Found!"); history.push("/search") }
            setErrand(res.data[0])
        }).catch(err => alert("Something went wrong!", err));
    };

    function sendOffer() {
        socket.emit("message", { content, fee, receiver: errand.poster, sender: user.id, type: "OFFER", errand: errand.id, state:0 });
    };
